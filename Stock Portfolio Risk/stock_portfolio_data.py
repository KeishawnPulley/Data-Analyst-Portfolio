...
# Filters to avoid the "minimum-salary rookie looks super efficient" distortion --
# a player making $12K who scores 2 ppg isn't actually a "value" find, the denominator
# is just tiny. These thresholds keep the ranking meaningful for real rotation players.
MIN_GAMES_PLAYED = 40
...
def merge_with_salary(stats_df, salary_csv_path=SALARY_CSV_PATH):
    ...
    # Check for and remove duplicate player rows (can happen from mid-season trades
    # appearing twice, or salary file quirks)
    dupe_count = merged["player_name"].duplicated().sum()
    if dupe_count > 0:
        print(f"Note: found {dupe_count} duplicate player row(s), keeping first occurrence.")
        merged = merged.drop_duplicates(subset="player_name", keep="first")

    return merged

def compute_value_metrics(df):
    # Filter out minimum-salary / low-minutes players so the ranking reflects
    # real rotation contributors, not small-sample outliers with tiny salaries
    before_count = len(df)
    df = df[
        (df["games_played"] >= MIN_GAMES_PLAYED) &
        (df["minutes_per_game"] >= MIN_MINUTES_PER_GAME) &
        (df["salary"] >= MIN_SALARY)
    ].copy()
    print(f"Filtered to rotation players (>= {MIN_GAMES_PLAYED} GP, "
          f">= {MIN_MINUTES_PER_GAME} MPG, >= ${MIN_SALARY:,} salary): "
          f"{len(df)}/{before_count} players remain.")
